...
void BinaryExpr::genCode()
{
    ...
    if (op == AND)
    {
        ...
        bb->addSucc(trueBB);
        expr1->genCode();
        // 把trueBB回填到表达式1节点的truelist中
        // 为了根据 expr1 的计算结果来决定后续控制流的正确路径
...
void IfStmt::genCode()
{
    ...
    // 流程图里：then_bb->end_bb , 当前bb->end_bb ，每个箭头都需要同时设置前驱和后继bb

    if(cond != nullptr)
    cond->genCode();
    if(!cond -> getOperand() -> getType() -> isBool())// 如果cond 表达式不是bool类型，需要转换一下
    {
    ...
    // 设置 then_bb（代表 if 语句的 then 分支的基本块）为当前的指令插入点。所有新生成的中间表示指令都将被添加到这个基本块中
    builder->setInsertBB(then_bb);
    // 生成的中间代码都会被放到then_bb里
    thenStmt->genCode();
...
void DeclStmt::genCode()
{
    ...
    for(auto iter = ids->Ids.rbegin(); iter != ids->Ids.rend(); iter++)
    {
        ...
        if(se->isGlobal())
        {
            ...
            bool temp = false;
            Operand *src;
            //检查是否有初始化赋值。如果有，生成该赋值语句的代码；如果没有，创建一个表示零值的常量
    ...
    for(long unsigned int i = 0; i < ids -> Assigns.size(); i++)
    {
        ...
        if(se -> isLocal())//对于局部变量，获取其地址并设置符号条目中的地址操作数
        {
...
void FuncFParams::genCode()//函数形参列表
{
    ...
    for(long unsigned int i = 0; i < FPs.size(); i++)
    {
        ...
        // 创建一个新的 PointerType（type1）和 IntType（type2）对象。PointerType 用于表示形参的类型，IntType 可能用于表示形参地址的类型。
        Type *type1 = new PointerType(se->getType());
        Type *type2 = new IntType(32);
        // 创建两个 TemporarySymbolEntry 对象和对应的 Operand 对象。这些用于在栈上为形参分配空间。
        ...
        // 在函数的入口基本块 entry 中插入一个 AllocaInstruction 指令，用于在函数栈上为形参分配空间。
        BasicBlock *entry = func->getEntry();
        Instruction *alloca;
...
void WhileStmt::genCode()
{
    ...
    cond -> genCode();
    if(!cond -> getOperand() -> getType() -> isBool())// 考虑类型转换
    {
        BasicBlock* bb=cond->builder->getInsertBB();
        Operand *src = cond->getOperand();
        SymbolEntry *se = new ConstantSymbolEntry(TypeSystem::intType, 0);
